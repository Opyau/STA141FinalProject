...
# Github link:[STA141AFinalProject Repository](https://github.com/Opyau/STA141FinalProject)
---
...
What we can immediately observe is that various countries have randomly distrubuted starting points of GDP growth/contraction (mean near zero) when cases are zero.  This will make finding an intercept more difficult.
However, we can observe a rising trend of GDP growth as cases rise.  Higher GDP is obviously not a causal effect of increased COVID cases, but there could still be significant correlation. 

```{r}
reg18 <- lm( GDPQTR ~ casespc)
summary(reg18)

```

Our regression shows the coefficient for Cases per capita has a positive correlation with percent change in GDP, to a significance of < .001
GDP appears to grow alongside COVID cases, possibly due to the dramatic drop in GDP at the start of the pandemic, resulting in GDP recovering while Covid cases continue to rise.
In addition, quarentines may have created new demand for consumer goods, while government economic stimulus kept consumer spending high.
The R-squared value is low, .0967
...
The residuals vs fitted graph is problematic, we have non-constant variance and nonlinearity, which suggests changes to our model might be neccesary.

Our data is normally distrubuted

The scale-location plot also is showing some issues, the magnitude of our residuals have too much variance.
